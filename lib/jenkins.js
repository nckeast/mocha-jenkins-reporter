...
function Jenkins(runner, options) {
  ...
  options.junit_report_nested_suites = process.env.JUNIT_REPORT_NESTED_SUITES || options.junit_report_nested_suites;
  ...
  function genSuiteReport() {
    ...
    let nestedSuite = false;
    if (options.junit_report_nested_suites) {
      if ((currentSuite.suite.parent != undefined) && (currentSuite.suite.parent.title.length > 0)) {
        // Identify embedded suite position.
        searchSuites = jsonResults.testsuites;

        // Build series of parent suites.
        let suitePath = [];
        let thisSuite = currentSuite.suite;
        while (thisSuite.parent) {
          suitePath.push(thisSuite.parent);
          thisSuite = thisSuite.parent
        }

        while (suitePath.length > 0) {
          thisSuite = suitePath.pop()
          searchSuites.some(function (object) {
            // Check object represents a test suite with attributes defined.
            if ((object.testsuite) && (object.testsuite[0])) {
              // Check for test suite name matching parent suite title.
              if (object.testsuite[0]._attr.name == thisSuite.fullTitle()) {
                if (suitePath.length == 0) {
                  // Full path matched.
                  nestedSuite = true
                  object.testsuite.push(testSuite);
                } else {
                  // Continue search with embedded suites.
                  searchSuites = object.testsuite
                  return true;
                }
              }
            }
  ...
  function endSuite() {
    if ((currentSuite != null) && (currentSuite.suite.title.length)) {
